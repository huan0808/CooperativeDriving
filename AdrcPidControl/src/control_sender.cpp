...
ControlSender::ControlSender(){
	...
	*/
    if (ENABLE_LOG) {
		...
		strftime(name_buffer, 80, "/home/nvidia/tmp/jluData/testData/vehicle_log__%F_%H%M%S.csv", &time_tm);
...
bool ControlSender::StartWrite(){
    ...
    lon_controller.SetCruiseSpeed(20.0); // set cruise speed [km/h]
    SendResetFrame();
    while (ros::ok) {
        rw_lock_.lock(); // ???
        ...
        /* const double steer_angle_command = lat_controller.ComputeSteerAngle(vehicle_info_.x, vehicle_info_.y, vehicle_info_.theta, vehicle_info_.steer_angle);
