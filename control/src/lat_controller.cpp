...
LatController::LatController() {
    ...
    char file_name[] = "/home/nvidia/tmp/reference/vehicle_log__2018-01-29_005246.csv";
    ...
    if (path_file == nullptr) {
        ...
        fscanf(path_file, "%s %s %s %s %s %s %s %s %s %s", 
               dummy_header_0, dummy_header_1, dummy_header_2, dummy_header_3, dummy_header_4, dummy_header_5, dummy_header_6,dummy_header_7,dummy_header_8,dummy_header_9);
        for (int i = 0; i < PATH_LEN; i++) {
            double t, x, y, theta, speed, steer_angle, front_steer_angle,lateral_error,heading_error;
	        int index;
            fscanf(path_file, "%lf %lf %lf %lf %lf %lf %lf %d %lf %lf", &t, &x, &y, &theta, &speed, &steer_angle, &front_steer_angle,&index,&lateral_error,&heading_error);
...
double LatController::ComputeSteerTorque(double x, double y, double theta, double curr_steer_angle) {
    ...
    lateral_error = cos_ref_heading * dy - sin_ref_heading * dx;
    // transform theta to (-pi, pi] radians, pointing to East as zero, increasing counter clockwise
    theta = NormalizeAngle(M_PI_2 - theta / 180.0 * M_PI);
...
double LatController::ComputeSteerAngle(double x, double y, double theta, double curr_steer_angle) {
    ...
    const double steer_angle_desired = -k_lat * lateral_error - k_heading * heading_error + ref_steer_angle;
    sequence_++;
    return  steer_angle_desired * 180 / M_PI;;
}
